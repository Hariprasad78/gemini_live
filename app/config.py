...
class AppConfig:
    ...
    ir_repeat_delay_seconds: float = 0.3
    ir_device_id: str = "samsung_tv_default"
    ir_dataset_path: str = str(Path("artifacts/ir_dataset.json"))
    agent_max_steps: int = 30
    agent_step_delay_seconds: float = 2.2
    agent_ui_settle_seconds: float = 3.2
    agent_observe_seconds: float = 1.5
    agent_min_visual_confidence: float = 0.75
    visual_brightness: float = 0.7
    visual_contrast: float = 0.7
    video_width: int = 1280
    video_height: int = 720
    video_frame_max_size: int = 1600
    video_jpeg_quality: int = 92
    video_send_interval_seconds: float = 1.5
